...
<?php if ($this->session->has_userdata('username')): ?>
   ...
   <?php if (!empty($this->session->flashdata('status'))): ?>
    ...
    <?php endif ?>
     <?= form_open('MyController/SearchRange', ['method' => 'POST']) ?>
                ...
                <table id="tabellaporan" class="table table-striped table-bordered" style="width:100%">
                    ...
                    foreach ($laporan as $l) :?>
                        <tr>
                            <td><?= $i++ ?></td>
                            <td><?= $l['nm_member'] ?></td>
                            <td><?= $l['alamat_member'] ?></td>
                            <td><?= $l['kode_invoice'] ?></td>
                            <td><?= $l['tgl_transaksi'] ?></td>
                            <td><?= $l['tgl_bayar'] ?></td>
                            <td><?= $l['batas_waktu'] ?></td>
                            <td><?= $l['total_harga'] ?></td>
                            <td><?= $l['status_transaksi'] ?></td>
                        </tr>
                        <?php $total = $total + $l['total_harga']; ?>
                    <?php endforeach ?>
                    </tbody>
            </table>
            <label><b>Total Pemasukan adalah <?= number_format($total , 0, ',', '.') ?></b></label>
            </div>
        </div>
    </div>
